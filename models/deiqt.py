...
class Experts_MOS(nn.Module):
    ...
    def forward(self, x, ref, Q_prev=None):
        ...
        if Q_prev is not None:
            Q_prev = Q_prev.view(B, 1, 1)
            Q_prev = Q_prev.expand(B, self.juery, 1)
            Q_prev = self.prev_aware(Q_prev)
            output_embedding = output_embedding + Q_prev
...
class deiqt_models(nn.Module):
    ...
    def __init__(
        ...
        self.max_score = 100  # M = 100
        self.num_iterations = 3
        self.step_lambda = nn.Parameter(torch.tensor(0.1))  # 可学习的优化步长
