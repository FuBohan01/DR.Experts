...
class LoRALayer():
    ...
    def merge_lora_param(self):
        ...
        for param_name, lora_name in self.params_with_lora.items():
            ...
            p_new = p.detach() + self.merge_BA(param_name) * self.scaling * p.detach()
            set_param(self, param_name, param=p_new, mode='update')

    def add_lora_data(self):
        r"""NOT differentiable"""
        for param_name, lora_name in self.params_with_lora.items():
            eval(f'self.{param_name}').data += self.merge_BA(param_name) * self.scaling * eval(f'self.{param_name}').data
...
class PlainMultiheadAttentionLoRA(nn.Module, LoRALayer):
    def __init__(
            self,
    ...
    def forward(self,
        ...
        if self.r > 0 and not self.merged:
            self.merge_lora_param()
            result = self.forward_module(query, key, value, **kwargs) 
            self.sub_lora_data()
            return result
        else:
