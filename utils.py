...
def load_pretrained(config, model, logger):
    ...
    checkpoint = torch.load(
        config.MODEL.PRETRAINED, map_location="cpu", weights_only=True
    )
...
if __name__ == "__main__":
    ...
    # 模拟数据
    y_pred = torch.rand(16, dtype=torch.float16, device="cuda")  # 模拟 FP16 数据
    y = torch.rand(16, dtype=torch.float16, device="cuda")

    # 在 AMP 模式下调用 plcc_loss
    with torch.amp.autocast("cuda", enabled=True):
        loss = plcc_loss(y_pred, y)
